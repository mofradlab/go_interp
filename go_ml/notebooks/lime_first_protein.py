...
si, ei = 4, 29
bert_distr = torch.stack(bert_distr)[:, :, si:ei]
bert_distr /= bert_distr.sum(dim=2, keepdim=True)
bert_attr = torch.nan_to_num((torch.log(bert_distr)*bert_distr).sum(axis=2), 0) #Negative entropy of residue positions
bert_attr = bert_attr - 1e9*(~enzyme_mask)
...
def mut_batch(prot_id, seq, key_ind, lambda_=15, batch_size=5000):
...
for i, r in enumerate(enzyme_iterator()):
    mut_seq_batch = mut_batch(r['prot_id'], r['seq'], top_k_ind[i, :min(75, len(r['seq']))], lambda_=15, batch_size=5000)
...
with open('/home/andrew/GO_interp/data/first_enzyme_5000_mut_evals.pkl', 'wb') as f:
